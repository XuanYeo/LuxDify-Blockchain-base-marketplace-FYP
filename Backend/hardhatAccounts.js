...
// Track which accounts are already assigned
const assignedAccounts = new Set();

// Function to get a free account
export function getUnassignedAccount() {
  for (const account of hardhatAccounts) {
    if (!assignedAccounts.has(account.address)) {
      assignedAccounts.add(account.address);
      return account;
    }
  }
  
  // If all accounts are assigned, generate a new random account
...
// Function to check if an account is already assigned
export function isAccountAssigned(address) {
  return assignedAccounts.has(address);
}

// Initialize assigned accounts from database
export function initializeAssignedAccounts(addresses) {
  addresses.forEach(address => {
    if (address) {
      assignedAccounts.add(address);
    }
  });
}

// clear set
export function keepOnlyFirstAssignedAccount() {
  const first = Array.from(assignedAccounts)[0,1];
  assignedAccounts.clear();
  if (first) {
    assignedAccounts.add(first);
  }
}
