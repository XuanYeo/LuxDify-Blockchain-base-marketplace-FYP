...
const CollectionPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:3000/collection/jewelry")
      .then((res) => {
        if (res.data.success) {
    ...
    <div>
      ...
      <div className="collection-wrapper">
        ...
        <aside className="filter-sidebar">
          <h3>Categories</h3>
          <ul>
            <li><Link href="/admin/collection" className="dropdown_a">All</Link></li>
            <li><Link href="/admin/collection/category_bag" className="dropdown_a">Bag</Link></li>
            <li><Link href="/admin/collection/category_jewelry" className="dropdown_a">Jewelry</Link></li>
        ...
        <div className="collection-container">
          <h2 className="section-title highlight-title">LuxDify Collection - Jewelry</h2>
          <div className="product-card-grid">
            {products.map((product: any) => {
              const isOwner = userId === product.Owner_ID;
              const isFavorite = favoriteIds.includes(product.Product_ID);
              return (
                <Link href={`/admin/product/${product.Product_ID}`} key={product.Product_ID} className="product-card" style={isOwner ? { boxShadow: "0 0 8px red" } : {}}>
